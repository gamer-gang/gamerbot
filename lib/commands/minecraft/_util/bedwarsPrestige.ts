...
export const prestigeColors: { [key: string]: Color[] } = {
  0: [gray],
  100: [white],
  200: [gold],
  300: [aqua],
...
export const getPrestigePalette = (level: number): Color[] => {
  ...
  for (const prestige of keys) {
    if (level >= parseInt(prestige)) return prestigeColors[prestige]
  }
...
export const getPrestigePlaintext = (player: Player): string => {
  ...
  const star = getStarSymbol(level)
...
export const drawPrestige = (c: SKRSContext2D, player: Player): number =>
  transaction(c, () => {
    ...
    const level = Math.floor(
      getLevelForExp(player.stats.Bedwars.Experience ?? 0)
    )
    ...
    const width = split.reverse().reduce(
      (offset, segment) => {
        ...
        return (
          offset -
          (segment.text === star ? c.measureText(star).width * 1.1 : charWidth)
        )
      },

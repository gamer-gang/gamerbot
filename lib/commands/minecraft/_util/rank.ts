...
export const getRank = (player: Player): Rank => {
  ...
  for (const key of ['monthlyPackageRank', 'newPackageRank', 'packageRank']) {
    ...
    if (
      rank != null &&
      (out == null ||
        (rankWeights[rank as Rank] ?? 0) > (out != null ? rankWeights[out] : 0))
    ) {
      out = rank as Rank
    }
  }
...
export const drawRank = (
  ...
  const split = parseFormattedText(rankPrefixes[rank]).map((segment) => {
    if (/^\*+$/.test(segment.text)) {
      segment.color =
        colors[
          (player.rankPlusColor ?? 'red').toLowerCase() as keyof typeof colors
        ]
  ...
  const prefixWidth = split.reduce((offset, segment) => {
    ...
    const adjustedCharWidth = segment.text.includes('⚝')
      ? s.getCharWidth(c, '⚝')
      : charWidth

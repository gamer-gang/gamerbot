import type { Bedwars, Player } from '@calico32/hypixel-types'
import { Image, createCanvas } from '@napi-rs/canvas'
import _ from 'lodash'
import hash from 'object-hash'
import * as s from '../../style.js'
import { insertUuidDashes } from '../../util.js'
import { StatsProviderResponse, statsProvider } from './_statsProvider.js'
import {
  drawPrestige,
  getExpForLevel,
  getLevelForExp,
...
function findCanvasWidth({
  ...
  const gamemodeColumnWidth = Math.max(
    ...Object.keys(rows).map((row) => tc.measureText(row).width)
  )
    ...
    return (
      Math.max(
        ...
        ...Object.keys(rows).map(
          (mode) => tc.measureText(stats[mode][col].toString()).width
        )
      ) +
      s.padding * 0.75
    )
  ...
  const totalSubheaders = Math.max(
    leftSubheaders.length,
    rightSubheaders.length
  )
  ...
  const width = Math.round(
    Math.max(
      ...
      // subheader
      (avatar?.width ?? 0) +
        s.getCharWidth(s.subheaderHeight) * totalSubheaderChars,
      // data
      totalColumnWidth
    )
  )
...
const STATS_PROVIDER_BEDWARS = statsProvider('bedwars', {
  ...
  async makeStats(player, avatar) {
    ...
    const dataHash = hash(
      { stats: apiData, avatar: avatar?.src.buffer },
      { algorithm: 'md5' }
    )
    ...
    for (const row in rows) {
      ...
      for (const col in rowData) {
        ...
        stats[row][col] =
          stringVal ||
          (isFinite(rowData[col]) ? rowData[col].toLocaleString() : '-')
      }
    }
    ...
    const bblr = s.round(
      (apiData.beds_broken_bedwars ?? 0) / (apiData.beds_lost_bedwars ?? 0)
    )
    ...
    transaction(c, () => {
      ...
      c.fillText(
        rightHeader,
        rightHeaderOffset - s.getCharWidth(c) * 2,
    ...
    transaction(c, () => {
      ...
      for (const [i, mode] of Object.keys(rows).entries()) {
        const lineY =
          i * (s.mainHeight + s.padding * 2) + s.mainHeight + 2 * s.padding
      ...
      transaction(c, () => {
        ...
        for (const [i, col] of statColumns) {
          ...
          c.lineTo(
            x - statColumnWidths[i] - s.padding,
            height - c.getTransform().f
          )
          c.stroke()

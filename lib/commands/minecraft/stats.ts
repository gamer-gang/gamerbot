import type { HypixelCacheResponse } from '@calico32/hypixel-cache'
...
const COMMAND_STATS = command(ApplicationCommandType.ChatInput, {
  ...
  async run(context) {
    ...
    assert(
      identifier,
      'Expected identifier to be set after checking input and database'
    )
    ...
    const provider = STATS_PROVIDERS.find(
      (provider) => provider.name === gamemode
    )
    ...
    try {
      ...
      const response = await fetch(
        `${env.HYPIXEL_CACHE_URL}/${inputType}/${identifier}`,
        {
      ...
      if (!responseData.success) {
        await interaction.editReply({
          embeds: [
            Embed.error(responseData.error ?? JSON.stringify(responseData)),
          ],
        })
        return CommandResult.Failure
      }
        ...
        await interaction.editReply({
          embeds: [
            Embed.error('Could not find player.').setFooter({
              text: `data ${Math.round(timers.data / 1000)}ms (cache ${
                timers.cache
              }ms)`,
            }),
          ],
      ...
      const { image, metadata } = await provider.makeStats(
        player,
        avatarResponse.status >= 200 && avatarResponse.status < 300
          ? avatar
          : undefined
      )
      ...
      await interaction.editReply({
        embeds: debug
          ? [Embed.info('Debug information', Formatters.codeBlock(debugInfo))]
          : [],
